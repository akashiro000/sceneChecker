...
class SceneChecker:
    ...
    def run_checks(self, selected_checks, progress_callback=None):
        ...
        for i, check in enumerate(selected_checks):
            ...
            check_method = getattr(self, check["function"], None)
            if check_method and callable(check_method):
                results = check_method(check)
                # 複数の結果を返す場合に対応
    ...
    def adjust_lamina_faces(self, items):
        """ラミナフェースを削除"""
        if items:
            cmds.select(items)
            cmds.delete()
    ...
    def adjust_zero_edge_length(self, items):
        """長さゼロのエッジをマージ"""
        if items:
            for edge in items:
                try:
    ...
    def adjust_zero_area_faces(self, items):
        """面積ゼロのフェースを削除"""
        if items:
            cmds.select(items)
            cmds.delete()
    ...
    def adjust_non_frozen_transforms(self, items):
        """トランスフォームをフリーズ"""
        if items:
            cmds.select(items)
    ...
    def adjust_negative_scale(self, items):
    ...
    def adjust_unused_influences(self, items):
        """未使用のインフルエンスを削除"""
        if items:
            for item in items:
                try:
    ...
    def check_geometry_issues(self, check_info):
        """ジオメトリの問題をまとめてチェック"""
        results = []
    ...
    def check_ngons(self, check_info):
        """N-gonをチェック"""
        meshes = cmds.ls(type="mesh", long=True)
        ngons = []
    ...
    def check_zero_area_faces(self, check_info):
        """面積ゼロのフェースをチェック"""
        meshes = cmds.ls(type="mesh", long=True)
        zero_faces = []
    ...
    def check_uv_issues(self, check_info):
        """UVの問題をまとめてチェック"""
        results = []
        meshes = cmds.ls(type="mesh", long=True)
    ...
    def check_missing_textures(self, check_info):
    ...
    def check_texture_sequences(self, check_info):
        """テクスチャシーケンスの問題をチェック"""
        import os
        import re
    ...
    def check_naming_issues(self, check_info):
    ...
    def check_bg_naming_convention(self, check_info):
        """BG専用の厳格な命名規則チェック
    ...
    def check_transform_issues(self, check_info):
    ...
    def check_joint_orientation(self, check_info):
        """ジョイントの向きをチェック"""
    ...
    def check_skin_weights(self, check_info):
        """スキンウェイトの問題をチェック"""
        results = []
    ...
    def check_unused_influences(self, check_info):
        """未使用のインフルエンスをチェック"""
        skin_clusters = cmds.ls(type="skinCluster")
        unused = []
    ...
    def check_animation_keys(self, check_info):
        """アニメーションキーの問題をチェック"""
    ...
    def check_shader_issues(self, check_info):
